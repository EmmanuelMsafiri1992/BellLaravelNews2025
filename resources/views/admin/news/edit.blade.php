...
<body>
                                ...
                                <div class="card-body">
                                    <form id="newsForm" method="POST"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea name="title" id="title" class="form-control" rows="3">{{ old('title', $news->title ?? '') }}</textarea>
                                            <div class="d-flex justify-content-between align-items-center mt-1">
                                                <div class="text-danger-tinymce" id="title-error-tinymce"></div>
                                                <small id="title-counter" class="text-muted">0 / 80 words</small>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            <label for="description" class="form-label">Description <span
                                                    class="text-danger">*</span></label>
                                            <textarea name="description" id="description" class="form-control" rows="10">{{ old('description', $news->description ?? '') }}</textarea>
                                            <div class="d-flex justify-content-between align-items-center mt-1">
                                                <div class="text-danger-tinymce" id="description-error-tinymce"></div>
                                                <small id="description-counter" class="text-muted">0 / 200 words</small>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-4">
                                            <label class="form-label">{{ __('Display on Screens') }}</label>
                                            <div class="border rounded p-3">
                                                @forelse($screens as $screen)
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="screens[]"
                                                               value="{{ $screen->id }}" id="screen_{{ $screen->id }}"
                                                               {{ $news->screens->contains($screen->id) ? 'checked' : '' }}>
                                                        <label class="form-check-label" for="screen_{{ $screen->id }}">
                                                            {{ $screen->name }}
                                                            @if($screen->location)
                                                                <small class="text-muted">({{ $screen->location }})</small>
                                                            @endif
                                                        </label>
                                                    </div>
                                                @empty
                                                    <p class="text-muted mb-0">{{ __('No screens available. Create screens first.') }}</p>
                                                @endforelse
                                            </div>
                                            <small class="text-muted">{{ __('Select which screens should display this news') }}</small>
                                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Function to update word counter based on TEXT content only (not HTML)
            function updateCharacterCounter(editor) {
                const content = editor.getContent({format: 'text'}); // Gets plain text without HTML
                const wordCount = countWords(content);
                const counter = document.getElementById(editor.id + '-counter');

                if (!counter) return;

                const maxWords = editor.id === 'title' ? 80 : 200;
            ...
            // Initialize TinyMCE with full formatting capabilities
            if (typeof tinymce !== 'undefined') {
                tinymce.init({
                    selector: '#description, #title',
                    plugins: 'advlist autolink lists link image charmap preview anchor pagebreak nonbreaking insertdatetime wordcount fullscreen code',
                    toolbar: 'undo redo | formatselect | bold italic underline strikethrough | fontsize | forecolor backcolor | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | removeformat | link image | code | fullscreen',
                    height: 250,
                    menubar: false,
                    ...
                    setup: function(editor) {
                        editor.on('change keyup', function() {
                            ...
                            // Update character counter with TEXT content only
                            updateCharacterCounter(editor);
                        });
                ...
                console.error("TinyMCE not loaded. Please ensure the script is accessible.");
            }
            ...
            if (newsForm) {
                newsForm.addEventListener('submit', function(event) {
                    ...
                    // Validate title
                    const titleTextContent = titleEditor ? titleEditor.getContent({format: 'text'}) : '';
                    const titleWordCount = countWords(titleTextContent);
                    if (!titleEditor || titleTextContent.trim() === '') {
                        titleTextarea.classList.add('is-invalid-tinymce');
                        titleErrorDiv.textContent = 'The title field is required.';
                        isValid = false;
                    } else if (titleWordCount > 80) {
                        titleTextarea.classList.add('is-invalid-tinymce');
                        titleErrorDiv.textContent = 'Title has ' + titleWordCount + ' words. Maximum is 80 words (text only).';
                        isValid = false;
                    } else {
                    ...
                    const descTextContent = descriptionEditor ? descriptionEditor.getContent({format: 'text'}) : '';
                    const descWordCount = countWords(descTextContent);
                    if (!descriptionEditor || descTextContent.trim() === '') {
                        ...
                        descriptionTextarea.classList.add('is-invalid-tinymce');
                        descriptionErrorDiv.textContent = 'Description has ' + descWordCount + ' words. Maximum is 200 words (text only).';
                        isValid = false;
                    } else {
                    ...
                    // Validate other standard required fields (category_id, status)
                    ...
                    fetch(this.action, {
                        ...
                        .catch(error => {
                            ...
                            if (error.errors) {
                                let errorMessage = '<strong>Validation Failed:</strong><ul class="mb-0 mt-2">';
                                ...
                                showAlert('danger', `Failed to update news article: ${error.message}`);
                            } else {
                                showAlert('danger', 'Failed to update news article. Unknown error.');
                            }
                        });
                });
            }
