...
class NewsController extends Controller
{
    ...
    public function create()
    {
        try {
            ...
            // Check if screens table exists before querying
            $screens = collect([]);
            try {
                $screens = Screen::active()->get();
            } catch (\Exception $screenException) {
                Log::warning('Screens table not available: ' . $screenException->getMessage());
            }

            return view('admin.news.create', compact('categories', 'screens'));
        } catch (\Exception $e) {
    ...
    {
        try {
            // Strip HTML tags for validation to count only text content
            $titleText = strip_tags($request->title);
            $descriptionText = strip_tags($request->description);

            // Count words (split by whitespace and filter empty strings)
            $titleWordCount = count(array_filter(preg_split('/\s+/', trim($titleText))));
            $descriptionWordCount = count(array_filter(preg_split('/\s+/', trim($descriptionText))));

            // Validate word count
            if ($titleWordCount > 80) {
                return response()->json([
                    'success' => false,
                    'message' => 'Validation failed',
                    'errors' => ['title' => ['The title has ' . $titleWordCount . ' words but maximum is 80 words (text only, excluding formatting tags).']]
                ], 422);
            }
            ...
            if ($descriptionWordCount > 200) {
                return response()->json([
                    'success' => false,
                    'message' => 'Validation failed',
                    'errors' => ['description' => ['The description has ' . $descriptionWordCount . ' words but maximum is 200 words (text only, excluding formatting tags).']]
                ], 422);
            }
            ...
            // Attach screens to news
            if ($request->has('screens') && is_array($request->screens)) {
                $news->screens()->attach($request->screens);
            }
    ...
    public function edit(News $news)
    {
        try {
            ...
            return view('admin.news.edit', compact('news', 'categories', 'screens'));
        } catch (\Exception $e) {
    ...
    public function update(Request $request, News $news)
    {
        try {
            ...
            if ($request->has('screens')) {
                $news->screens()->sync($request->screens);
            } else {
                $news->screens()->detach();
            }
