...
class NetworkManager
{
    ...
    private function applyNetplanConfig($interface, $config)
    {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'netplan_');
        file_put_contents($tempFile, $yaml);

        // Use sudo to move and set permissions
        shell_exec("sudo mv $tempFile $netplanFile 2>&1");
        shell_exec("sudo chmod 0600 $netplanFile 2>&1");
    ...
    {
        // Read current content using sudo
        $currentContent = shell_exec("sudo cat {$this->dhcpcdFile} 2>&1");
        if ($currentContent === null || $currentContent === false) {
            $currentContent = '';
        }

        // Remove existing static config for this interface
        $content = preg_replace("/interface $interface.*?(?=\n\n|\ninterface|\z)/s", '', $currentContent);
        ...
        // Write to temporary file first
        $tempFile = tempnam(sys_get_temp_dir(), 'dhcpcd_');
        file_put_contents($tempFile, $content);

        // Use sudo to move the file
        shell_exec("sudo mv $tempFile {$this->dhcpcdFile} 2>&1");
    ...
    private function applyInterfacesConfig($interface, $config)
    {
        ...
        // Write to temporary file first
        $tempFile = tempnam(sys_get_temp_dir(), 'interfaces_');
        file_put_contents($tempFile, $content);

        // Use sudo to move the file
        shell_exec("sudo mv $tempFile {$this->interfacesFile} 2>&1");
    ...
    private function restartNetworking()
    {
        try {
            if ($this->networkSystem === 'netplan') {
                ...
                shell_exec('sudo systemctl restart dhcpcd 2>&1');
            } else {
                shell_exec('sudo systemctl restart networking 2>&1');
            }
