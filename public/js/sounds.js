...
export async function populateSounds() {
    ...
    try {
        ...
        const soundFilenames = (backendSounds.sounds || []).map(s => typeof s === "object" ? s.filename : s);
        setSounds(soundFilenames);
...
export function testSound(sound) {
    ...
    audio.play().catch(e => {
        ...
        showFlashMessage(`Failed to play sound: ${e.message}. Your browser might have blocked autoplay or the file is corrupted.`, "error", 'dashboardFlashContainer');
    });
}
